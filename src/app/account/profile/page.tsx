...
export default function ProfilePage() {
  ...
  const handleLinkProvider = async (provider: 'google' | 'facebook') => {
    ...
    try {
      ...
      if (error) throw error;
      // Redirect will happen automatically — no need to reset state
    } catch (err: any) {
      alert(err.message || 'Failed to link account. Please try again.');
          ...
          <div className="flex-1 w-full lg:max-w-[calc(100%-312px)]">
            ...
            {activeTab === 'profile' && (
              <div className="space-y-4">
                ...
                >
                  <div>
                    <h2 style={{ fontSize: '20px', fontWeight: 700, color: '#111827' }}>Connected Accounts</h2>
                    <p style={{ fontSize: '14px', color: '#6B7280', marginTop: '4px' }}>
                      Link multiple login methods — sign in with email or Google using the same account
                    </p>
                  </div>
                  ...
                  <div className="space-y-3 pt-4 border-t border-gray-100">
                    ...
                    <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div style={{ fontSize: '14px', fontWeight: 700, color: '#111827' }}>Email / Password</div>
                          <div style={{ fontSize: '12px', color: '#6B7280' }}>{user?.email}</div>
                        </div>
                      </div>
                      <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-bold bg-green-50 text-green-700 border border-green-100">
                    ...
                    {/* Google */}
                    {(() => {
                      const isGoogleLinked = user?.identities?.some((i: any) => i.provider === 'google');
                      return (
                        <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-white border border-gray-200 flex items-center justify-center shadow-sm">
                            ...
                            <div>
                              <div style={{ fontSize: '14px', fontWeight: 700, color: '#111827' }}>Google</div>
                              <div style={{ fontSize: '12px', color: '#6B7280' }}>
                                {isGoogleLinked ? 'Signed in with Google is enabled' : 'Not connected yet'}
                              </div>
                            </div>
                          </div>
                          {isGoogleLinked ? (
                            ...
                            <button
                              ...
                              style={{
                                ...
                                padding: '0 16px',
                                borderRadius: '12px',
                                background: linkingProvider === 'google' ? '#E5E7EB' : '#111827',
                                color: 'white',
                                fontSize: '13px',
                                fontWeight: 600
                              }}
                              className="flex items-center gap-2 transition-all cursor-pointer disabled:opacity-50"
                            >
                    ...
                    {/* Facebook */}
                    {(() => {
                      const isFacebookLinked = user?.identities?.some((i: any) => i.provider === 'facebook');
                      return (
                        <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-white border border-gray-200 flex items-center justify-center shadow-sm">
                            ...
                            <div>
                              <div style={{ fontSize: '14px', fontWeight: 700, color: '#111827' }}>Facebook</div>
                              <div style={{ fontSize: '12px', color: '#6B7280' }}>
                                {isFacebookLinked ? 'Signed in with Facebook is enabled' : 'Not connected yet'}
                              </div>
                            </div>
                          </div>
                          {isFacebookLinked ? (
                            ...
                            <button
                              ...
                              style={{
                                ...
                                padding: '0 16px',
                                borderRadius: '12px',
                                background: linkingProvider === 'facebook' ? '#E5E7EB' : '#1877F2',
                                color: 'white',
                                fontSize: '13px',
                                fontWeight: 600
                              }}
                    ...
                    <p className="text-[11px] text-gray-400 pt-1">
                      💡 Linking accounts lets you sign in with any connected method and see all your orders in one place.
                    </p>
                  </div>
                </div>
              </div>
